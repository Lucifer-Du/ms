    ...
    <Card>
        ...
        <Card class="section" :title="card_title">
            <template v-if="list.length > 0">
                ...
                <template v-else>
                    <List>
                        <ListItem v-for="(course, cindex) in list" :key="cindex">
                            <Space size="large" split>
                                <span>学科：{{ course.course_name }}</span>
                                <span>最高分：{{ course.max }}</span>
                                <span>学生人数：{{ course.total }}</span>
                                <span>及格人数：{{ course.pass }}</span>
                                <span>不及格人数：{{ course.total - course.pass }}</span>
                            </Space>
                        </ListItem>
                    </List>
                </template>
        ...
        queryData() {
            ...
            post("/api/user/query_userinfo", {
                ...
                if (code === 1) {
                    ...
                    switch (access) {
                        case "student":
                            const { user_id, sum, avg, rank, ...course } = data;
                        ...
                        case "teacher":
                            ...
                            data.forEach(item => {
                                ...
                                if (names.includes(item.course_name)) {
                                    ...
                                    teacher[index].data.push({ user_name, course_record: Number(course_record) });
                                }
                                else {
                                    names.push(course_name);
                                    teacher.push({ course_name, data: [{ user_name, course_record: Number(course_record) }] });
                                }
                            });
                            teacher.map(item => {
                                item.max = item.data.reduce((pre, cur) => {
                                    if (cur.course_record > pre) {
                                        pre = cur.course_record;
                                    }
                                    return pre;
                                }, 0);
                                item.total = item.data.length;
                                item.pass = item.data.reduce((pre, cur) => {
                                    if (cur.course_record >= 60) {
                                        pre ++;
                                    }
                                    return pre;
                                }, 0);
