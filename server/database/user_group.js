...
class user_group {
    ...
    static all(callback) {
    	// 使用sqlite3的all
        db.all('SELECT * FROM user_group', callback);
    }
    // 添加数据
    static create(data, callback) {
        ...
        db.run(sql, data.name, data.role, callback);
    }
    // 根据id 获取数据
    static find(id, callback) {
        if (!id) return callback(new Error(`缺少参数id`));
    	// 使用sqlite3的get
        db.get('SELECT * FROM user_group WHERE id = ?', id, callback);
    }
    // 更新数据
    ...
    static delete(id, callback) {
        if (!id) return callback(new Error(`缺少参数id`));
        db.run('DELETE FROM user_group WHERE id=?', id, callback)
    }
}
